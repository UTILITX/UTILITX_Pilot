...
import { MapPin, Edit } from "@/lib/icons"

type Props = {
  records: RequestRecord[]
  showActions?: boolean
  onGeoreference?: (payload: { recordId: string; fileId: string }) => void
  onRedrawGeometry?: (payload: { recordId: string; fileId: string }) => void
  showOrg?: boolean
}
...
export function RecordsTable({
  ...
  return (
    <Table>
      <TableCaption>Files contributed to this space.</TableCaption>
      <TableHeader>
        <TableRow>
          ...
          {showOrg ? <TableHead>Org</TableHead> : null}
      ...
      <TableBody>
        {rows.map((r) => (
          <TableRow key={r.file.id}>
            ...
            {showOrg ? <TableCell className="whitespace-nowrap">{r.record.orgName || "-"}</TableCell> : null}
            <TableCell className="whitespace-nowrap">{r.record.uploaderName}</TableCell>
            ...
            {showActions ? (
              <TableCell className="text-right">
                <div className="flex gap-2 justify-end">
                  ...
                  {r.file.status === "Georeferenced" && onRedrawGeometry && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => onRedrawGeometry({ recordId: r.record.id, fileId: r.file.id })}
                    >
